...
import { useIsTimePassedSync, useProfessionalBookings } from "@/hooks/useBookings"
...
export function WeeklySchedule({ professional }: WeeklyScheduleProps) {
  ...
  const professionalBookings = useProfessionalBookings(professional.id)
        ...
        <div className="space-y-4">
          {Object.entries(schedule).map(([day, slots]) => (
            <DaySchedule
              ...
              slots={slots}
              selectedSessionType={selectedSessionType}
              professionalBookings={professionalBookings.data || []}
              isTimePassed={useIsTimePassedSync}
            />
          ))}
        </div>
      </CardContent>
